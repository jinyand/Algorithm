...
- 인형이 들어있는 칸을 판별하기 위해서는 뽑기 기계가 세로로 내려와야한다.  
따라서 (0, 0) (1, 0) (2, 0) ... 의 순으로 체크해야하므로 board[ j ][ moves[i] - 1 ] 가 item이 들어있는 칸이 된다.
- 스택을 사용하여 방금 넣은 item과 일치할 경우 해당 item을 pop으로 삭제시키고, 해당 값을 0으로 초기화 시켜준다.
- 그 외의 경우엔 집어든 item을 스택에 넣어준다.
